...
import { Sparkles, X, MessageSquare, Send, Bot, User, Loader2 } from 'lucide-react';
...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'bot', text: string }[]>([
    { role: 'bot', text: "Hello! I'm Tripetrip's AI guide. Ready to discover some hidden gems?" }
  ]);
  ...
  const handleSend = async () => {
    ...
    const currentMessages = [...messages, { role: 'user', text: userMessage }] as any;
    setInput('');
    setMessages(currentMessages);
    setIsTyping(true);

    const response = await chatWithAI(currentMessages);
    
    setIsTyping(false);
    setMessages(prev => [...prev, { role: 'bot', text: response || "I'm sorry, I couldn't process that suggestion right now." }]);
  };
          ...
          >
            ...
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="flex items-center text-[10px] font-bold uppercase tracking-widest text-emerald-600">
            ...
            <div className="p-5 border-t border-slate-100 bg-slate-50/30">
              ...
              <p className="text-[9px] text-center mt-4 text-slate-300 font-bold uppercase tracking-widest">
                Gemini 1.5 Flash • Adventure Insights
              </p>
            </div>
          </motion.div>
        )}

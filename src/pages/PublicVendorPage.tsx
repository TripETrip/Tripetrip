...
import { Listing, VendorProfile } from '@/src/lib/firebase';
...
export default function PublicVendorPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!slug) return;
      try {
        const vendorQ = query(collection(db, 'vendor_profiles'), where('slug', '==', slug), limit(1));
        const vendorSnap = await getDocs(vendorQ);
        
        if (!vendorSnap.empty) {
          const vendorData = { id: vendorSnap.docs[0].id, ...vendorSnap.docs[0].data() } as VendorProfile;
          setVendor(vendorData);

          const listingsQ = query(collection(db, 'listings'), where('vendor_id', '==', vendorData.id), where('is_active', '==', true));

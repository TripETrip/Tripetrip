...
export default function Search() {
  ...
  const fetchListings = async () => {
    setLoading(true);
    try {
      let q = query(collection(db, 'listings'), where('is_active', '==', true));
      
      const category = searchParams.get('category');
      if (category) {
        const normalizedCategory = category.charAt(0).toUpperCase() + category.slice(1).toLowerCase();
        if (normalizedCategory === 'Stays') {
          q = query(q, where('category', 'in', ['Stays', 'Hotels']));
        } else {
      ...
      const data = querySnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as object) } as Listing));
      setListings(data);
    } catch (error) {

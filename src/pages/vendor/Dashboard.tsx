...
import { getVendorInsights } from '@/src/lib/gemini';
import { toast } from 'sonner';
import { cn, formatCurrency } from '@/src/lib/utils';
import { motion, AnimatePresence } from 'motion/react';
import { VendorProfile, Booking } from '@/src/lib/firebase';
...
export default function VendorDashboard() {
  ...
  const fetchVendorData = async () => {
    ...
    try {
      // Get vendor profile
      const vendorQ = query(collection(db, 'vendor_profiles'), where('user_id', '==', user.uid), limit(1));
      const vendorSnapshot = await getDocs(vendorQ);

      if (!vendorSnapshot.empty) {
        const vendorDoc = vendorSnapshot.docs[0];
        const profile = { id: vendorDoc.id, ...(vendorDoc.data() as object) } as VendorProfile;
        setVendorProfile(profile);
        setSlugInput(profile.slug || '');

        // Get listings
        const listingQ = query(collection(db, 'listings'), where('vendor_id', '==', profile.id));
        const listingSnapshot = await getDocs(listingQ);
        const listingData = listingSnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as object) }));
        setListings(listingData);

        // Get recent bookings (demo data for now if empty)
        const bookingQ = query(collection(db, 'bookings'), where('vendor_id', '==', profile.id), limit(10));
        const bookingSnap = await getDocs(bookingQ);
        if (bookingSnap.empty) {
          // Add demo bookings if none exist
          setBookings([
            { id: '1', traveler_name: 'Ankit Sharma', status: 'confirmed', total_price: 120, guests: 2, start_date: '2024-05-10', end_date: '2024-05-12', listing_id: 'l1', vendor_id: profile.id, traveler_id: 't1', created_at: '' },
            { id: '2', traveler_name: 'Sarah J.', status: 'pending', total_price: 45, guests: 1, start_date: '2024-05-15', end_date: '2024-05-15', listing_id: 'l2', vendor_id: profile.id, traveler_id: 't2', created_at: '' }
          ] as any);
        } else {
          setBookings(bookingSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Booking)));
        }
      }
    } catch (error) {
  ...
  const updateWebsiteSettings = async () => {
    ...
    try {
      if (slugInput !== vendorProfile.slug) {
        const q = query(collection(db, 'vendor_profiles'), where('slug', '==', slugInput));
      ...
      const docRef = doc(db, 'vendor_profiles', vendorProfile.id);
      await updateDoc(docRef, { slug: slugInput });
      setVendorProfile({ ...vendorProfile, slug: slugInput });
  ...
  const generateAIInsights = async () => {
    setIsAiLoading(true);
    const insight = await getVendorInsights({ listingsCount: listings.length, revenue: 4200, rating: 4.8 });
                    ...
                    <div className="flex items-center space-x-6">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-2">
                           <div className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Inventory: <span className="text-slate-900">{listing.specifics?.units || 1} Total</span></div>

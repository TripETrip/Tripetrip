...
import { Plane, Mail, Lock, Loader2, User, Building2, Phone } from 'lucide-react';
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        ...
        // 3. If vendor, create vendor_profiles document
        if (role === 'vendor') {
          await setDoc(doc(db, 'vendor_profiles', user.uid), {
            user_id: user.uid,
            business_name: `${fullName}'s Travel Services`,
            business_type: 'stays',
            created_at: serverTimestamp()
          });
        }

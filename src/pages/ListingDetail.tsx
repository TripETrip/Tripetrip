...
import { db, Listing } from '@/src/lib/firebase';
...
export default function ListingDetail() {
  ...
  const fetchListing = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        const listingData = { id: docSnap.id, ...(docSnap.data() as object) } as any;
        
        // Fetch vendor profile manually
        const vendorRef = doc(db, 'vendor_profiles', listingData.vendor_id);
        const vendorSnap = await getDoc(vendorRef);
        if (vendorSnap.exists()) {
          listingData.vendor_profiles = { id: vendorSnap.id, ...(vendorSnap.data() as object) };
        }
